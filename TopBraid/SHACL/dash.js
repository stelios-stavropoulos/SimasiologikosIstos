...
function validateHasValueIn($this, $path, $hasValueIn) {
	var set = new NodeSet();
	set.addAll(new RDFQueryUtil($shapes).rdfListToArray($hasValueIn));
	return $data.query().
			path($this, toRDFQueryPath($path), "?value").
			filter(function(sol) { return set.contains(sol.value) })
			hasSolution();
}
